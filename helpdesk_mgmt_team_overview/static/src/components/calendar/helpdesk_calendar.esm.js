...
// Nombres de modelo posibles para las etapas de ticket, probados en orden
// (distintas versiones/instalaciones de Helpdesk usan uno u otro).
const STAGE_MODEL_CANDIDATES = ["helpdesk.ticket.stage"];

// Claves de state que controlan la apertura de cada panel de filtro desplegable.
// Solo uno puede estar abierto a la vez (ver closeAllPanels()).
const PANEL_STATE_KEYS = ["teamFilterOpen", "contactFilterOpen", "stageFilterOpen"];
...
export class HelpdeskCalendar extends Component {
    ...
    setup() {
        ...
        this.lastSelectedStageIds = [...this.state.selectedStageIds];

        // Cierra los desplegables de filtro (equipos/contactos/etapas) al hacer click fuera de ellos.
        useExternalListener(document, "click", () => this.closeAllPanels());
        ...
        onWillStart(async () => {
            this.accessScope = await this._computeAccessScope();
            await this.loadUsersAndTickets();
        });
    }

    async _computeAccessScope() {
    ...
    /** Intenta leer las etapas probando, en orden, los posibles nombres de modelo. */
    async _loadStages() {
        for (const model of STAGE_MODEL_CANDIDATES) {
            try {
                return await this.orm.searchRead(model, [], ["name", "id"]);
            } catch (err) {
                // Modelo no disponible en esta instalación: probar el siguiente candidato.
            }
        }
        return [];
    }
    ...
    async loadUsersAndTickets() {
        let users = await this.orm.searchRead(
            "res.users",
            [
        ...
        let teams = await this.orm.searchRead(
            "helpdesk.ticket.team",
            [["ticket_control", "=", true]],
            ["name", "user_ids", "color"]
        );

        // Igual que en TeamPerformance._loadTeams: "team" y "own" solo ven
        // los equipos donde el usuario actual está incluido.
        if (this.accessScope === "team" || this.accessScope === "own") {
            teams = teams.filter((t) => t.user_ids.includes(this.user.userId));
        }

        // Igual que en TeamPerformance._buildTeamGroups: "own" recorta
        // los miembros de esos equipos a solo el propio usuario.
        if (this.accessScope === "own") {
            teams = teams.map((t) => ({
                ...t,
                user_ids: t.user_ids.filter((uid) => uid === this.user.userId),
            }));
        }

        const allowedUserIds = new Set(teams.flatMap((t) => t.user_ids));
        users = users.filter((u) => allowedUserIds.has(u.id));

        this.state.users = users;
        this.state.teams = teams;

        this.state.stages = await this._loadStages();

        const previousStageIds = this.state.selectedStageIds.length
            ? this.state.selectedStageIds
            : this.lastSelectedStageIds;
        const validStageIds = previousStageIds.filter((id) =>
            this.state.stages.some((stage) => stage.id === id)
        );

        if (validStageIds.length) {
            this.state.selectedStageIds = validStageIds;
        } else if (this.state.stages.length) {
            this.state.selectedStageIds = [this.state.stages[0].id];
        }
        this.lastSelectedStageIds = [...this.state.selectedStageIds];
    ...
    /** Un grupo por cada equipo (con sus miembros) */
    get groupedUsers() {
    ...
    // -------------------------------------------------------------------
    // Paneles de filtro (equipo / contacto / etapa)
    //
    // Los tres desplegables comparten la misma lógica de apertura (solo uno
    // abierto a la vez) y de selección múltiple sobre un array de ids,
    // así que esa lógica común vive en closeAllPanels() / togglePanel() /
    // toggleArrayItem().
    // -------------------------------------------------------------------

    /** Cierra los tres paneles de filtro. */
    closeAllPanels() {
    ...
    /** Abre/cierra un panel de filtro (identificado por su clave en state) y cierra los demás. */
    ...
    toggleStageFilterPanel(ev) {
        this.togglePanel("stageFilterOpen", ev);
    }

    toggleStageSelection(stageId, ev) {
        ev.stopPropagation();
        this.toggleArrayItem(this.state.selectedStageIds, stageId);
        this.lastSelectedStageIds = [...this.state.selectedStageIds];
    }
    ...
    /**
     * Tickets de un usuario, visibles en el período actual, filtrados por equipo,
     * contacto y etapa seleccionados, y ordenados por posición visual.
     * Todas las condiciones se evalúan en un único filter() (en vez de encadenar
     * varios .filter()) para recorrer el array de tickets una sola vez.
     */
    getUserTickets(userId, teamId) {
        const { selectedPartnerIds, selectedStageIds } = this.state;
        return this.state.tickets
            .filter((t) =>
                t.user_id && t.user_id[0] === userId &&
                (!teamId || !t.team_id || t.team_id[0] === teamId) &&
                this.isTicketVisibleInSelectedMonth(t) &&
                (!selectedPartnerIds.length || (t.partner_id && selectedPartnerIds.includes(t.partner_id[0]))) &&
                (!selectedStageIds.length || (t.stage_id && selectedStageIds.includes(t.stage_id[0])))
            )
            .sort((a, b) => {
